...
$wr_query = mysql_query("SELECT wr_id,wr_type,req_name,title,status,cc_staff,cc_staff2,cc_staff3,priority,ph_num,building,room_no,add_date FROM 
	workrequests WHERE (status = 'Finished' OR status = 'Cancelled' OR status = 'Followup') 
...
EOD;
$tot_query = mysql_query("SELECT COUNT(wr_id) FROM workrequests where add_date > '2010-07-01'");
$sub_query = mysql_query("SELECT COUNT(wr_id) FROM workrequests WHERE status = 'Submitted' and add_date > '2010-07-01'");
$app_query = mysql_query("SELECT COUNT(wr_id) FROM workrequests WHERE status = 'Approved' and add_date > '2010-07-01'");
$ip_query = mysql_query("SELECT COUNT(wr_id) FROM workrequests WHERE (status = 'Waiting' OR status ='Started') and add_date > '2010-07-01'");
$fin_query = mysql_query("SELECT COUNT(wr_id) FROM workrequests WHERE (status = 'Finished' OR status ='Cancelled'
	OR status = 'Closed') and add_date > '2010-07-01'");
