...
$wr_query = mysql_query("SELECT wr_id,wr_type,mfg,req_name,title,status,cc_staff,ph_num,building,room_no,add_date FROM 
	stu_wr WHERE (status = 'Submitted' OR status = 'Started' OR status = 'Waiting') 
	$name_cond $tech_cond $bldg_cond $type_cond ORDER BY $order_by,status");
echo "
...
echo "<TABLE BORDER=1>
	<TR>
		...
		<TD><FORM ACTION=\"$PHP_SELF\" METHOD=\"POST\">
		    ...
		    <INPUT TYPE=\"submit\" VALUE=\"WR Type/Mfg\">
		    </FORM></TD>
		<TD><FORM ACTION=\"$PHP_SELF\" METHOD=\"POST\">
...
while ($rows > 0):
	...
	<TR>
		...
		<TD>$wr_obj->wr_type<BR>$wr_obj->mfg</TD>
	...
	endwhile;
echo "</TABLE>";
$tot_query = mysql_query("SELECT COUNT(wr_id) FROM stu_wr where add_date > '2014-07-01'");
$sub_query = mysql_query("SELECT COUNT(wr_id) FROM stu_wr WHERE status = 'Submitted' and add_date > '2014-07-01'");
$ip_query = mysql_query("SELECT COUNT(wr_id) FROM stu_wr WHERE (status = 'Waiting' OR status ='Started') and add_date > '2014-07-01'");
$fin_query = mysql_query("SELECT COUNT(wr_id) FROM stu_wr WHERE (status = 'Finished' OR status ='Cancelled'
	OR status = 'Closed') and add_date > '2014-07-01'");
